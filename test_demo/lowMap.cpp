...
//
 ...
 //updated as occupied.
//将激光扫描的参数作为输入，并适当地更新世界地图
//startx和stary是激光扫描的起源，MeasuredDist是它行进的距离（线迹的长度），θ是线的角度（弧度）。 
//parentID让我们知道此更新与哪个粒子ID相关联，
//并且当激光扫描被对象停止时（而不是仅在没有看到任何东西的情况下行进最大范围）指示最后一个网格方块需要更新为已占用时，addEnd = 1。
void LowAddTrace(double startx, double starty, double MeasuredDist, double theta, int parentID, int addEnd) {
	double overflow, slope; // Used for actually tracing the line 用于实际跟踪线
	int x, y, incX, incY, endx, endy;
	int xedge, yedge;       // Used in computing the midpoint. Recompensates for which edge of the square the line entered from 用于计算中点。 重新补偿行输入的方块的哪个边缘
	double dx, dy;
	double distance, error;
	double secant, cosecant;   // precomputed for speed 预先计算速度
