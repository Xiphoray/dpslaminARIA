...
struct TPath_struct {
	...
	// D is the major axis of lateral motion, which is along the average facing angle during this time step 横向运动的主轴
	// C is the minor axis, which is rotated +pi from D   C是短轴，从D旋转+ pi
	// T is the angular change in facing angle.  面对角度的角度变化。
	float C, D, T;
...
// -size is the total available size of this dynamic array 动态数组可用大小
// -total indicates how many of those entries are currently being used. 正在使用的输入
//
