...
void AddToWorldModel(TSense sense, int particleNum) {
	...
	for (j = 0; j < SENSE_NUMBER; j++)
		// Normalize readings relative to the pose of current assumed position 相对于当前假定位置的姿势标准化读数
...
void PrintMap(char *name, TAncestor *parent, int particles, double overlayX, double overlayY, double overlayTheta)
{
	...
	fclose(printFile);
	imgflash = true;
	//const char* file_i = cimg_option("-i", sysCall, "Input image");
...
void InitLowSlam(ArRobot *robot)
{
	...
	curGeneration = 0;
	//if (PLAYBACK == "") {
		// Grab our initial reading of the odometer and laser 初次获取位置和传感器数值
...
//
void LowSlam(int &continueSlam, TPath **path, TSenseLog **obs, ArRobot *robot)
{
	double moveAngle;
	int counter;
	int i, j, overflow = 0;
	...
	ArLog::log(ArLog::Normal, "Initialize the worldMap");
	...
	ArLog::log(ArLog::Normal, "Initialize particle");
	...
	if (curGeneration == 0) {
		...
		curGeneration++;
		ArLog::log(ArLog::Normal, "curGeneration");
	}
	...
	// This greatly helps to eliminate some initial spurious values.
		...
		ArLog::log(ArLog::Normal, "move particle");
	}
	...
	ArLog::log(ArLog::Normal, "Get our observation log started");
	...
	while ((continueSlam) && (counter < LOW_DURATION)) {
		...
		// This way we have them coordinated.
			GetOdometry(odometry, &robot);
			ArLog::log(ArLog::Normal, "get odometry successful");
			overflow = 1;
		...
		if (overflow > 0) {
			...
			ArLog::log(ArLog::Normal, "get sensation data successful");
			...
			ArLog::log(ArLog::Normal, "localization procedure successful");
			...
			ArLog::log(ArLog::Normal, "Update Ancestry successful");
			...
			curGeneration++;
			counter++;
			ArLog::log(ArLog::Normal, "counter: %d", counter);
		...
		//	ArLog::log(ArLog::Normal, "print map successful");
		//}
		////?
	}
	...
	ArLog::log(ArLog::Normal, "Return particle path successful");
